...
export default function dashboard(){
    const [values, setValues] = useState({
        ...
        cotization: {_count: {policyNum : 0}},
        send: {_count: {policyNum : 0}},
        paid: {_count: {policyNum : 0}},
        names: [],
        tableData: [],
    });
    ...
    useEffect(() => {
        async function fetchValues()
        {
            ...
            const cotization = await getValues(user.token, 'policies/function/count/cotization');
            const send = await getValues(user.token, 'policies/function/count/send');
            const paid = await getValues(user.token, 'policies/function/count/paid');
    ...
    const {clients, policies, insured, cotization, send, paid, primes, names, tableData} = values;
    ...
    const params = [
      ...
      {
        title: "Asegurados",
        value: insured._sum.insuredValue ? `${insured._sum.insuredValue} Bs` : "0 Bs",
      ...
      },
      {
        ...
        value: primes._sum.primeValue ? `${primes._sum.primeValue} Bs` : "0 Bs",
    ...
    const params2 = [
      {
        ...
        value: cotization._count.policyNum ? `${cotization._count.policyNum}` : "0",
      ...
      },
      {
        ...
        value: send._count.policyNum ? `${send._count.policyNum}` : "0",
      ...
      },
      {
        ...
        value: paid._count.policyNum ? `${paid._count.policyNum}` : "0",
    ...
    return (
        <>
        <Layout title='Tablero' user = {user}>
